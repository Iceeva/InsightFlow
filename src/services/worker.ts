...
// ─── Redis Connection (plain object, pas d'instance ioredis) ────────────────
// On utilise ConnectionOptions natif de BullMQ pour éviter le conflit
// entre ioredis du projet et celui bundlé dans bullmq.
...
const eventWorker = new Worker(
  ...
  async (job) => {
    ...
    try {
      path = event.url ? new URL(event.url).pathname : null;
    } catch {
      path = null;
    }
...
const analyticsWorker = new Worker(
  ...
  async (job) => {
    ...
    if (type === 'anomaly_check') {
      ...
      if (anomalies.some((a: { severity: string }) => a.severity === 'high')) {
        ...
        if (project?.workspace.members[0]) {
          await notify({
            ...
            body:     `${anomalies.filter((a: { severity: string }) => a.severity === 'high').length} high-severity anomalies detected in ${project.name}`,
